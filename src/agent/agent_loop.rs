            ...
            match result {
                ...
                RespondResult::ToolCalls(tool_calls) => {
                    ...
                    if self.optimizer.should_inhibit(utility) && iteration > 1 {
                        tracing::info!("U-threshold inhibition triggered (U={:.3}), exiting loop early", utility);
                        return Ok(AgenticLoopResult::Response("I have completed the most relevant actions. Please let me know if you need anything else.".to_string()));
                    }
                }
            }
        }
