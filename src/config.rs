...
impl std::str::FromStr for LlmProviderType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s.to_lowercase().as_str() {
            "nearai" | "near" => Ok(Self::NearAi),
            "google" | "gemini" => Ok(Self::Google),
            _ => Err(format!(
                "invalid LLM provider '{}', expected 'nearai' or 'google'",
                s
            )),
        }
    }
...
pub struct GoogleConfig {
    ...
    /// Base URL for the Google AI API (OpenAI-compatible)
    pub base_url: String,
}
...
impl LlmConfig {
    fn from_env() -> Result<Self, ConfigError> {
        let provider = if let Some(p_str) = optional_env("LLM_PROVIDER")? {
            p_str.parse().map_err(|e| ConfigError::InvalidValue {
        ...
        let nearai_api_mode = if let Some(mode_str) = optional_env("NEARAI_API_MODE")? {
...
impl EmbeddingsConfig {
    fn from_env() -> Result<Self, ConfigError> {
        ...
        // Also load Google API key for embeddings if present
        let google_api_key = optional_env("GOOGLE_API_KEY")?.map(SecretString::from);

        // Priority: env var > settings > default
        // Default to "local" for sovereign mode if no other setting exists
        let provider = optional_env("EMBEDDING_PROVIDER")?
            .unwrap_or_else(|| {
                let p = settings.embeddings.provider.clone();
                if p == "nearai" || p == "openai" {
                    // Check if keys are actually present, otherwise fallback to local
                    if optional_env("OPENAI_API_KEY").unwrap_or(None).is_some() {
                        "openai".to_string()
                    } else if optional_env("NEARAI_API_KEY").unwrap_or(None).is_some() {
                         "nearai".to_string()
                    } else {
                        "local".to_string()
                    }
        ...
        let model_env = optional_env("EMBEDDING_MODEL")?;
        let model_setting = settings.embeddings.model.clone();

        // Determine model: Env > Settings
        let model = if let Some(m) = model_env {
            m
        } else {
            model_setting
        };
        ...
        let enabled = optional_env("EMBEDDING_ENABLED")?
            ...
            .unwrap_or_else(|| {
                // Check settings, or auto-enable if API key present, or if provider is local
                settings.embeddings.enabled 
                    || (provider == "openai" && openai_api_key.is_some()) 
                    || (provider == "google" && google_api_key.is_some())
                    || provider == "local"
            });
    ...
    /// Get the Google API key if configured.
...
impl AgentConfig {
    fn from_env() -> Result<Self, ConfigError> {
        ...
        Ok(Self {
            ...
            neco_arc_mode: optional_env("NECO_ARC_MODE")?
                .map(|s| s.parse())
                .transpose()
                .map_err(|e| ConfigError::InvalidValue {
