...
async fn main() -> anyhow::Result<()> {
    ...
    let embeddings: Option<Arc<dyn EmbeddingProvider>> = if config.embeddings.enabled {
        match config.embeddings.provider.as_str() {
            ...
            "local" => {
                tracing::info!("Initializing local embeddings (fastembed)...");
                match ironclaw::workspace::LocalEmbeddings::new() {
                    Ok(provider) => {
                         tracing::info!("Local embeddings initialized (model: all-MiniLM-L6-v2)");
                         Some(Arc::new(provider))
                    }
                    Err(e) => {
                        tracing::error!("Failed to initialize local embeddings: {}", e);
                        None
                    }
                }
            }
            "google" => {
                 tracing::info!(
                    "Embeddings enabled via Google (model: {})",
                    config.embeddings.model
                );
                if let Some(api_key) = config.embeddings.google_api_key() {
                     Some(Arc::new(
    ...
    let workspace = if let Some(ref s) = store {
        ...
        // Seed identity files from local filesystem if missing
        if let Err(e) = ws.seed_identity().await {
